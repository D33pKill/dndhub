...
const ESTADOS_RETRATO = [
  ...
  { key: 'en_zona', label: 'ESTADO ESPECIAL', desc: 'Fuerza especial activada', color: '#ffffff' },
...
export default function EditarPersonajePage() {
  ...
  // Carga inicial del estado clonado
  useEffect(() => {
    if (personaje && !localChar) {
      setLocalChar(JSON.parse(JSON.stringify(personaje)));
    }
  }, [personaje, localChar]);
    ...
    return (
      <div className="bg-dungeon min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="font-heading text-lg mb-4" style={{ color: '#6b1818', letterSpacing: '0.1em' }}>CARGANDO DATOS DEL PERSONAJE...</p>
          <div className="animate-spin w-8 h-8 border-2 border-t-transparent rounded-full mx-auto" style={{ borderColor: '#5a4e40', borderTopColor: 'transparent' }} />
        </div>
      </div>
    );
  }

  const subclases = SUBCLASES_POR_CLASE[localChar.clase] ?? [];

  const handleGuardar = async (targetChar?: any) => {
    const charToSave = targetChar || localChar;
    if (!charToSave) return;
    setGuardando(true);
    try {
      await actualizar(charToSave);
  ...
  const handleSubirRetrato = async (estadoKey: string, file: File) => {
    ...
    try {
      const url = await subirRetrato(localChar.id, estadoKey, file);
      setLocalChar((prev: any) => ({
        ...prev,
        retratos: { ...prev.retratos, [estadoKey]: url }
      }));
    } finally {
  ...
  const handleEliminarRetrato = (estadoKey: string) => {
    setLocalChar((prev: any) => {
      const nuevosRetratos = { ...prev.retratos };
      delete nuevosRetratos[estadoKey];
      return { ...prev, retratos: nuevosRetratos };
    });
  };
  ...
  const handleActualizarAccion = (accionId: string, campo: string, valor: string) => {
    setLocalChar((prev: any) => ({
      ...prev,
      acciones: prev.acciones.map((a: any) => a.id === accionId ? { ...a, [campo]: valor } : a)
    }));
  };
  ...
  const handleActualizarRasgo = (rasgoId: string, campo: string, valor: string) => {
    setLocalChar((prev: any) => ({
      ...prev,
      rasgos: prev.rasgos.map((r: any) => r.id === rasgoId ? { ...r, [campo]: valor } : r)
    }));
  };
  ...
  return (
    <div className="bg-dungeon min-h-screen flex flex-col">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b flex-shrink-0"
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-sm" style={{ background: localChar.color_acento }} />
            <span className="font-heading text-lg font-bold" style={{ color: localChar.color_acento }}>
              EDITAR: {localChar.nombre.toUpperCase()}
            </span>
          </div>
          <button
            onClick={() => handleGuardar()}
            className="btn-success px-5 py-2 rounded-sm text-xs font-bold transition-all cursor-pointer"
          >
      ...
      {/* Tabs */}
      <div className="flex border-b overflow-x-auto flex-shrink-0" style={{ borderColor: '#1c1712', background: 'rgba(6,4,2,0.5)' }}>
        {TABS.map(t => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            ...
            style={{
              background: tab === t.id ? `${localChar.color_acento}12` : 'transparent',
              borderBottom: `2px solid ${tab === t.id ? localChar.color_acento : 'transparent'}`,
              color: tab === t.id ? localChar.color_acento : '#5a4e40',
            }}
          >
            {t.label}
          </button>
        ...
        <div className="max-w-2xl mx-auto">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {tab === 'identidad' && (
                <>
                  <Campo label="NOMBRE" required>
                    <input type="text" value={localChar.nombre} maxLength={50}
                      onChange={e => setLocalChar({ ...localChar, nombre: e.target.value })}
                      className="hud-input w-full px-4 py-3 rounded-sm text-lg" />
                  </Campo>
                  <div className="grid grid-cols-2 gap-4">
                    <Campo label="CLASE">
                      <input type="text" value={localChar.clase}
                        onChange={e => setLocalChar({ ...localChar, clase: e.target.value })}
                        list="clases-list"
                    ...
                    <Campo label="SUBCLASE">
                      <input type="text" value={localChar.subclase}
                        onChange={e => setLocalChar({ ...localChar, subclase: e.target.value })}
                        list="subclases-list"
                        className="hud-input w-full px-3 py-3 rounded-sm" />
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Campo label="RAZA">
                      <input type="text" value={localChar.raza}
                        onChange={e => setLocalChar({ ...localChar, raza: e.target.value })}
                        list="razas-list"
                        className="hud-input w-full px-3 py-3 rounded-sm" />
                    ...
                    <Campo label="ALINEAMIENTO">
                      <select value={localChar.alineamiento}
                        onChange={e => setLocalChar({ ...localChar, alineamiento: e.target.value })}
                  ...
                  <Campo label="TRASFONDO">
                    <input type="text" value={localChar.trasfondo}
                      onChange={e => setLocalChar({ ...localChar, trasfondo: e.target.value })}
                      placeholder="Ej: Noble, Criminal, Soldado..."
                      className="hud-input w-full px-3 py-3 rounded-sm" />
                  </Campo>
                  <Campo label="NIVEL">
                    <div className="flex items-center gap-4">
                      <input type="range" min={1} max={20} value={localChar.nivel}
                        onChange={e => setLocalChar({ ...localChar, nivel: Number(e.target.value) })}
                        className="flex-1" />
                      <span className="font-heading text-2xl font-bold" style={{ color: localChar.color_acento }}>{localChar.nivel}</span>
                    </div>
                  </Campo>
                  <Campo label="COLOR DE ACENTO">
                    <div className="flex flex-wrap gap-3 items-center">
                      {COLORES_ACENTO.map(c => (
                        <button key={c} onClick={() => setLocalChar({ ...localChar, color_acento: c })}
                          className="w-8 h-8 rounded-sm cursor-pointer hover:scale-110 transition-transform"
                          style={{
                            background: c,
                            border: localChar.color_acento === c ? '2px solid white' : '2px solid transparent',
                            boxShadow: localChar.color_acento === c ? `0 0 12px ${c}` : 'none',
                          }} />
                      ))}
                      <input type="color" value={localChar.color_acento}
                        onChange={e => setLocalChar({ ...localChar, color_acento: e.target.value })}
                ...
                <>
                  <div className="grid grid-cols-2 gap-4">
                    <Campo label="HP ACTUAL">
                      <input type="number" value={localChar.hp} min={0} max={localChar.hp_max}
                        onChange={e => setLocalChar({ ...localChar, hp: Number(e.target.value) })}
                    ...
                    <Campo label="HP MÁXIMO">
                      <input type="number" value={localChar.hp_max} min={1}
                        onChange={e => setLocalChar({ ...localChar, hp_max: Number(e.target.value) })}
                        className="hud-input w-full px-3 py-3 rounded-sm font-heading text-center"
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <Campo label="CLASE DE ARMADURA">
                      <input type="number" value={localChar.ca} min={1}
                        onChange={e => setLocalChar({ ...localChar, ca: Number(e.target.value) })}
                        className="hud-input w-full px-3 py-3 rounded-sm font-heading text-center"
                        style={{ color: '#7a5818', fontSize: '18px' }} />
                    </Campo>
                    <Campo label="VELOCIDAD (pies)">
                      <input type="number" value={localChar.velocidad} min={0} step={5}
                        onChange={e => setLocalChar({ ...localChar, velocidad: Number(e.target.value) })}
                        className="hud-input w-full px-3 py-3 rounded-sm font-heading text-center"
                        style={{ color: '#3a4870', fontSize: '18px' }} />
                    </Campo>
                    <Campo label="INICIATIVA">
                      <input type="number" value={localChar.iniciativa}
                        onChange={e => setLocalChar({ ...localChar, iniciativa: Number(e.target.value) })}
                        className="hud-input w-full px-3 py-3 rounded-sm font-heading text-center"
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <Campo label="BON. COMPETENCIA">
                      <input type="number" value={localChar.bonificador_competencia}
                        onChange={e => setLocalChar({ ...localChar, bonificador_competencia: Number(e.target.value) })}
                        className="hud-input w-full px-3 py-3 rounded-sm font-heading text-center"
                        style={{ color: '#7a5818', fontSize: '18px' }} />
                    </Campo>
                    <Campo label="BON. ATAQUE">
                      <input type="number" value={localChar.bonificador_ataque}
                        onChange={e => setLocalChar({ ...localChar, bonificador_ataque: Number(e.target.value) })}
                        className="hud-input w-full px-3 py-3 rounded-sm font-heading text-center"
                        style={{ color: '#6b1818', fontSize: '18px' }} />
                    </Campo>
                    <Campo label="BON. MAGIA">
                      <input type="number" value={localChar.bonificador_magia}
                        onChange={e => setLocalChar({ ...localChar, bonificador_magia: Number(e.target.value) })}
                        className="hud-input w-full px-3 py-3 rounded-sm font-heading text-center"
                        style={{ color: '#3a4870', fontSize: '18px' }} />
                    </Campo>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <Campo label="CA ESTADO ESPECIAL (opcional)">
                      <input type="number" value={localChar.ca_especial ?? ''}
                        onChange={e => setLocalChar({ ...localChar, ca_especial: e.target.value ? Number(e.target.value) : null })}
                        placeholder="Ej: 16"
                        className="hud-input w-full px-3 py-3 rounded-sm font-heading text-center"
                        style={{ color: '#7a5818' }} />
                    </Campo>
                    <Campo label="DADO ESPECIAL (opcional)">
                      <input type="text" value={localChar.dado_especial ?? ''}
                        onChange={e => setLocalChar({ ...localChar, dado_especial: e.target.value || null })}
                        placeholder="Ej: 2d6 (ataque furtivo)"
                        className="hud-input w-full px-3 py-3 rounded-sm" />
                    </Campo>
                  </div>
                  <Campo label="NOMBRE DEL ESTADO ESPECIAL">
                    <input type="text" value={localChar.nombre_estado_especial}
                      onChange={e => setLocalChar({ ...localChar, nombre_estado_especial: e.target.value })}
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {ATRIBUTOS_BASE.map(attr => {
                      const val = localChar.estadisticas[attr.key as keyof EstadisticasBase];
                        ...
                        <div key={attr.key} className="p-4 rounded-sm"
                          ...
                          <input type="range" min={1} max={30} value={val}
                            onChange={e => setLocalChar({
                              ...localChar,
                              estadisticas: { ...localChar.estadisticas, [attr.key]: Number(e.target.value) }
                            })}
                            className="w-full" />
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-2">
                    <Campo label="VIRTUDES / VENTAJAS (una por línea)">
                      <textarea
                        value={localChar.ventajas.join('\n')}
                        onChange={e => setLocalChar({ ...localChar, ventajas: e.target.value.split('\n').map(s => s.trim()).filter(Boolean) })}
                        className="hud-input w-full px-3 py-2 rounded-sm resize-none text-sm"
                        rows={4} />
                    </Campo>
                    <Campo label="MALDICIONES / DESVENTAJAS (una por línea)">
                      <textarea
                        value={localChar.desventajas.join('\n')}
                        onChange={e => setLocalChar({ ...localChar, desventajas: e.target.value.split('\n').map(s => s.trim()).filter(Boolean) })}
                        className="hud-input w-full px-3 py-2 rounded-sm resize-none text-sm"
                        rows={4} />
                    </Campo>
                  </div>
                ...
                <>
                  ...
                  <div className="space-y-2">
                    {HABILIDADES_DND.map(hab => {
                      const activa = !!localChar.habilidades[hab.nombre];
                      const entrada = localChar.habilidades[hab.nombre];
                      return (
                        <div key={hab.nombre} className="flex items-center gap-3 p-2 rounded-sm"
                          style={{
                            background: activa ? `${localChar.color_acento}08` : 'transparent',
                            border: `1px solid ${activa ? `${localChar.color_acento}40` : '#2e2820'}`,
                          }}>
                          <button
                            ...
                            style={{
                              background: activa ? localChar.color_acento : 'transparent',
                              border: `1px solid ${activa ? localChar.color_acento : '#5a4e40'}`,
                          ...
                          {activa && (
                            <>
                              <input type="number" value={entrada?.bonus ?? 0}
                                ...
                                style={{ color: localChar.color_acento, fontSize: '12px' }} />
                              <button
                                onClick={() => handleHabilidadExperto(hab.nombre)}
                                className="px-2 py-1 rounded-sm hud-label cursor-pointer"
                                style={{
                                  background: entrada?.experto ? `${localChar.color_acento}20` : 'transparent',
                                  border: `1px solid ${entrada?.experto ? localChar.color_acento : '#3d3028'}`,
                                  color: entrada?.experto ? localChar.color_acento : '#5a4e40',
                                  fontSize: '8px',
                                }}>
                                EXP
                              </button>
                  ...
                  <div className="mt-6">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {ATRIBUTOS_BASE.map(attr => {
                        const tieneGuardado = localChar.salvaciones[attr.label] !== undefined;
                        return (
                          <div key={attr.key} className="flex items-center gap-2 p-2 rounded-sm"
                            ...
                            <button
                              onClick={() => {
                                const nuevas = { ...localChar.salvaciones };
                                if (tieneGuardado) { delete nuevas[attr.label]; }
                                else { nuevas[attr.label] = 0; }
                                setLocalChar({ ...localChar, salvaciones: nuevas });
                              }}
                            ...
                            {tieneGuardado && (
                              <input type="number" value={localChar.salvaciones[attr.label] ?? 0}
                                onChange={e => setLocalChar({
                                  ...localChar,
                                  salvaciones: { ...localChar.salvaciones, [attr.label]: Number(e.target.value) }
                                })}
              ...
              {tab === 'acciones' && (
                <>
                  {(localChar.acciones ?? []).map((accion: any, i: number) => (
                    <div key={accion.id} className="p-4 rounded-sm" style={{ background: 'rgba(12,10,7,0.8)', border: '1px solid #2e2820' }}>
                      <div className="flex justify-between mb-3">
                        <span className="hud-label" style={{ color: localChar.color_acento }}>ACCIÓN {i + 1}</span>
                        <button onClick={() => setLocalChar({ ...localChar, acciones: localChar.acciones.filter((a: any) => a.id !== accion.id) })}
                  ...
                  <button onClick={handleAgregarAccion}
                    ...
                    style={{ background: `${localChar.color_acento}06`, border: `1px dashed ${localChar.color_acento}40`, color: localChar.color_acento, fontFamily: 'Cinzel, serif', fontSize: '12px' }}>
                    + AGREGAR ACCIÓN
                  </button>
                </>
              )}
              ...
              {tab === 'rasgos' && (
                <>
                  ...
                  {(localChar.rasgos ?? []).map((rasgo: any, i: number) => (
                    <div key={rasgo.id} className="p-4 rounded-sm" style={{ background: 'rgba(12,10,7,0.8)', border: '1px solid #2e2820' }}>
                      <div className="flex justify-between mb-3">
                        <span className="hud-label" style={{ color: localChar.color_acento }}>RASGO {i + 1}</span>
                        <button onClick={() => setLocalChar({ ...localChar, rasgos: localChar.rasgos.filter((r: any) => r.id !== rasgo.id) })}
                          className="hud-label cursor-pointer hover:opacity-70" style={{ color: '#8b2020' }}>✕</button>
                      </div>
                      <Campo label="NOMBRE">
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-4">
                    <Campo label="EQUIPO (uno por línea)">
                      <textarea
                        value={localChar.equipo.join('\n')}
                        onChange={e => setLocalChar({ ...localChar, equipo: e.target.value.split('\n').map(s => s.trim()).filter(Boolean) })}
                        className="hud-input w-full px-3 py-2 rounded-sm resize-none text-sm"
                        rows={6}
                    ...
                    <Campo label="IDIOMAS (uno por línea)">
                      <textarea
                        value={localChar.idiomas.join('\n')}
                        onChange={e => setLocalChar({ ...localChar, idiomas: e.target.value.split('\n').map(s => s.trim()).filter(Boolean) })}
                        className="hud-input w-full px-3 py-2 rounded-sm resize-none text-sm"
                        rows={6}
                        placeholder="Común&#10;Élfico" />
                    </Campo>
              ...
              {tab === 'retratos' && (
                <div className="grid grid-cols-2 gap-4">
                  {[
                    ...
                    { key: 'afectado', label: 'AFECTADO', desc: 'Bajo condición grave', color: '#39ff14' },
                    { key: 'inconsciente', label: 'INCONSCIENTE', desc: 'HP = 0', color: '#555' },
                    { key: 'en_zona', label: localChar.nombre_estado_especial ? localChar.nombre_estado_especial.toUpperCase() : 'ESTADO ESPECIAL', desc: 'Fuerza especial activada', color: '#ffffff' },
                    { key: 'shock', label: 'SHOCK / FALLO', desc: 'Backfire mágico', color: '#ff6d00' },
                  ].map(estado => (
                    <div key={estado.key} className="rounded-sm overflow-hidden"
                      ...
                      <div className="relative h-40" style={{ background: 'rgba(12,10,7,0.8)' }}>
                        {localChar.retratos[estado.key] ? (
                          <>
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img
                              src={localChar.retratos[estado.key]!}
                              alt={estado.label}
                      ...
                      <label className="block p-2 cursor-pointer hover:opacity-80 transition-opacity"
                        ...
                        <div className="mt-2 text-center hud-label py-1 rounded-sm"
                          ...
                          {localChar.retratos[estado.key] ? 'CAMBIAR' : 'SUBIR'}
                        </div>
                      </label>
                    </div>
                  ))}
              ...
              {tab === 'lore' && (
                <>
                  <Campo label="HISTORIA DEL PERSONAJE">
                    <textarea value={localChar.historia}
                      onChange={e => setLocalChar({ ...localChar, historia: e.target.value })}
                  ...
                  <Campo label="DESCRIPCIÓN FÍSICA">
                    <textarea value={localChar.apariencia}
                      onChange={e => setLocalChar({ ...localChar, apariencia: e.target.value })}
                      className="hud-input w-full px-4 py-3 rounded-sm resize-none lore-box"
                      rows={5}
                      style={{ fontFamily: 'Crimson Pro, serif', fontSize: '14px', color: '#b8a070' }}
                  ...
                  <Campo label="PERSONALIDAD / IDEALES / VÍNCULOS / DEFECTOS">
                    <textarea value={localChar.personalidad}
                      onChange={e => setLocalChar({ ...localChar, personalidad: e.target.value })}
                      className="hud-input w-full px-4 py-3 rounded-sm resize-none lore-box"
                      rows={5}
                      style={{ fontFamily: 'Crimson Pro, serif', fontSize: '14px', color: '#b8a070' }}
          ...
          >
            ...
            >
              ...
              <p className="font-lore text-sm mb-6" style={{ color: '#7a6e60' }}>
                Esta acción eliminará permanentemente a <strong style={{ color: '#b8a070' }}>{localChar.nombre}</strong> y todos sus datos.
              </p>
